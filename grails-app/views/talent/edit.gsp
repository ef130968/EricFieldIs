    ...
    <g:form method="post">
        ...
        <div class="dialog">
            <table>
                ...
                <tr class="prop">
                    <td valign="top" class="name">
                        <label for="description"><g:message code="talent.description.label"
                ...
                <tr class="prop">
                    ...
                    <td valign="top" class="value ${hasErrors(bean: talentInstance, field: 'profile', 'errors')}">
                        <g:select name="profile.id" from="${ericfieldis.Profile.list()}" optionKey="id"
                                  value="${talentInstance?.profile?.id}"/>
                    </td>
                </tr>

                <tr class="prop">
                    <td valign="top" class="name">
                        <label for="skills"><g:message code="talent.skills.label" default="Skills"/></label>
                    </td>
                    <td valign="top" class="value ${hasErrors(bean: talentInstance, field: 'skills', 'errors')}">

                        <ul>
                            <g:each in="${talentInstance?.skills?}" var="s">
                                <li><g:link controller="skill" action="show"
                                            id="${s.id}">${s?.encodeAsHTML()}</g:link></li>
                            </g:each>
                        </ul>
                        <g:link controller="skill" action="create"
                                params="['talent.id': talentInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'skill.label', default: 'Skill')])}</g:link>

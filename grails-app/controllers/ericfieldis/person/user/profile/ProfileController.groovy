...
class ProfileController {
    ...
    def create = { ProfileCommand cmd ->
        render(view: '/person/user/profile/create', model: [cmd: cmd, persons: Person.findAll(), profileTypes: ProfileType.list()])
    }

    def save = { ProfileCommand cmd ->
        if(cmd.hasErrors()) {
            return render(view: '/person/user/profile/create', model: [cmd: cmd, persons: Person.findAll(), profileTypes: ProfileType.list()])
        } else {
            def profileInstance = new Profile().update(cmd)
            if (profileInstance.save(flush: true)) {
                flash.message = "${message(code: 'opening.save.message.created')}"
